...
from hailtop.auth import service_auth_headers  # type: ignore
...
from ci.utils import reserve_namespace
...
GROUP BY active_namespaces.namespace''',
    ...
    services_per_namespace = {r['namespace']: [s for s in json.loads(r['services']) if s is not None] for r in records}

    internal_namespace_oauth_callbacks = {
        r['namespace']: r['oauth2_callback_url'] for r in records if r['namespace'] != 'default'
    }
    ...
    for proxy in ('gateway', 'internal-gateway'):
        ...
        cds = create_cds_response(proxy, default_services, services_per_namespace)
        rds = create_rds_response(proxy, default_services, services_per_namespace, internal_namespace_oauth_callbacks)
