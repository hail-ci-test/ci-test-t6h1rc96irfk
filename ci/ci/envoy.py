...
def create_rds_response(
    ...
    internal_services_per_namespace: Optional[Dict[str, List[str]]] = None,
    internal_namespace_oauth_callbacks: Optional[Dict[str, str]] = None,
) -> dict:
    internal_services_per_namespace = internal_services_per_namespace or {}
    internal_namespace_oauth_callbacks = internal_namespace_oauth_callbacks or {}
    ...
    if len(internal_services_per_namespace) > 0:
        hosts.append(internal_host(internal_services_per_namespace, internal_namespace_oauth_callbacks))
    return {
        'version_info': 'dummy',
...
def gateway_internal_host(
    services_per_namespace: Dict[str, List[str]], internal_namespace_oauth_callbacks: Dict[str, str]
) -> dict:
    internal_host = {
        '@type': 'type.googleapis.com/envoy.config.route.v3.VirtualHost',
        'name': 'internal',
    ...
    oauth_callback_rewrites = [
        {
            'match': {'prefix': urlparse(oauthcallback).path},
            'route': {'cluster': f'{namespace}-auth', 'prefix_rewrite': f'/{namespace}/auth/oauth2callback'},
        }
        for namespace, oauthcallback in internal_namespace_oauth_callbacks.items()
    ]
    internal_host['routes'].extend(oauth_callback_rewrites)
